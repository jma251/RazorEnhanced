...
namespace Assistant
{
    ...
    internal class Targeting
    {
        ...
        private static void TargetResponse(PacketReader p, PacketHandlerEventArgs args)
        {
            ...
            // The target has been used or cancelled, so a targeted spell is
            // finished with. This is the player's own answer going to the
            // server - an observed fact, not a prediction, and it does not
            // care about Faster Casting or anything the shard does
            // differently.
            //
            // Spells that never ask for a target produce no response and fall
            // back to the cast time from Config/spells.json, which is the one
            // thing here that cannot be observed.
